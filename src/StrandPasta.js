...
import {strandPasta} from './PastaData'; 
import RecipeCard from './RecipeCard'; 
import Card from './Card';
function StrandPasta(){
    ...
    //Adding id. 
    strandPasta.map((data, i) => ({id:i, ...data}));
    ...
    return(
        <div>
            ...
            {strandPasta.map((item) => <Card pastaName = {item.name} pastaIMG = {item.imageURL} onClick = {(e) => {setClickedPasta(item.name); getRecipeData(item.name); 
                }}/> )}
            </div>
        </div>
