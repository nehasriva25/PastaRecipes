...
import {ribbonPasta} from './PastaData'; 
import RecipeCard from './RecipeCard'; 
import Card from './Card';
function RibbonPasta(){
    ...
    //Adding id. 
    ribbonPasta.map((data, i) => ({id:i, ...data}));
    ...
    return(
        <div>
            ...
            {ribbonPasta.map((item) => <Card pastaName = {item.name} pastaIMG = {item.imageURL} onClick = {(e) => {setClickedPasta(item.name); getRecipeData(item.name); 
                }}/> )}
            </div>
        </div>
