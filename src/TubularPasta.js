...
import {tubularPasta} from './PastaData'; 
import RecipeCard from './RecipeCard'; 
import Card from './Card';
function TubularPasta(){
    ...
    //Adding id. 
    tubularPasta.map((data, i) => ({id:i, ...data}));
    ...
    return(
        <div>
            ...
            {tubularPasta.map((item) => <Card pastaName = {item.name} pastaIMG = {item.imageURL} onClick = {(e) => {setClickedPasta(item.name); getRecipeData(item.name); 
                }}/> )}
            </div>
        </div>
