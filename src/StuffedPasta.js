...
import {stuffedPasta} from './PastaData'; 
import RecipeCard from './RecipeCard'; 
import Card from './Card';
function StuffedPasta(){
    ...
    //Adding id. 
    stuffedPasta.map((data, i) => ({id:i, ...data}));
    ...
    return(
        <div>
            ...
            {stuffedPasta.map((item) => <Card pastaName = {item.name} pastaIMG = {item.imageURL} onClick = {(e) => {setClickedPasta(item.name); getRecipeData(item.name); 
                }}/> )}
            </div>
        </div>
