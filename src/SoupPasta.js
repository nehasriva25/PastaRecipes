...
import {soupPasta} from './PastaData'; 
...
function SoupPasta(){
    ...
    //Adding id. 
    soupPasta.map((data, i) => ({id:i, ...data}));
    ...
    return(
        <div>
            ...
            {soupPasta.map((item) => <Card pastaName = {item.name} pastaIMG = {item.imageURL} onClick = {(e) => {setClickedPasta(item.name); getRecipeData(item.name); 
                }}/> )}
            </div>
        </div>
